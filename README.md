...
| Sticky bar now reveals on scroll | It was permanently on, sitting on top of the hero's "See the work" button on phones. Now it appears once the hero scrolls away and hides again over the contact section. Implemented as a plain passive scroll listener after two failed attempts: `IntersectionObserver` batches entries during fast scrolling and reading `entries[0]` acted on a stale observation (bar stuck visible); replacing it with a `requestAnimationFrame` latch was worse (bar stuck hidden) because a hidden or throttled tab drops the queued frame and the "already scheduled" flag never clears. The current version holds no state that can wedge and re-syncs on `visibilitychange`, `pageshow`, `load` and `resize`. |
